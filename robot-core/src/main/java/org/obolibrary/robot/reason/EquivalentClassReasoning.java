 ...
 *
 * All the pieces that are needed to perform reasoning on equivalent classes in
 * an OWLOntology
 */
public class EquivalentClassReasoning {
    ...
    /**
     * The mode that represents a particular equivalent class reasoning
     * strategy, once found
     */
    ...
    /**
     * Growable Set of Equivalent Class Axioms that are found by the reasoner
     * that also fail the test in the above reasoningTest. These are Equivalent
     * Classes that are not allowed.
     */
    private Set<OWLEquivalentClassesAxiom> failingAxioms;

    /**
     * Growable set of found Equivalenct Class Axioms. As the reasoner finds
     * equivalent classes, they are added here
     */
    private Set<OWLEquivalentClassesAxiom> foundEquivalentAxioms;

    /**
     * Creates a new instance that will reason over the given ontology with the
     * strategy specified by the EquivalentClassReasoningMode.
     *
     ...
     * @param mode One of ALL, NONE, or ASSERTED_ONLY, representing a strategy
     *        to deal with found equivalent classes
     */
    public EquivalentClassReasoning(OWLOntology ontology, OWLReasoner reasoner,
    ...
    /**
     * Uses the reasoner to find all equivalent classes in the ontology, and
     * then adds the axioms that are not allowed the the list of failed axioms.
     * Once reasoning has occurred, a report can be logged with #logReport
     *
     * @return True if there were no failing equivalence axioms found,
     *         False if any failed the test
     */
    public boolean reason() {
        ...
        for (OWLClass c : ontology.getClassesInSignature()) {
            ...
            foundEquivalentAxioms.addAll(this.createEquivalenceAxioms(c,
                    equivalentClasses));
        }
    ...
    /**
     * Uses the provided logger to print out the results of the reasoning
     * operation.
     *
    ...
    private Set<OWLEquivalentClassesAxiom> createEquivalenceAxioms(OWLClass c,
            Set<OWLClass> equivalentClasses) {
        Set<OWLEquivalentClassesAxiom> equivalenceAxioms = new HashSet<>();
        for (OWLClass ec : equivalentClasses) {
            equivalenceAxioms.add(dataFactory.getOWLEquivalentClassesAxiom(c,
                    ec));
        }
     ...
     * @param mode one of the enum instances that define how reasoning behavior
     *        should be over equivalent axioms
     * @param ontology Ontology to reason over
     * @return The EquivalentAxiomReasoningTest instance with the implementation
     *         of how to detect a failing Equivalence Axiom
     */
    private static EquivalentAxiomReasoningTest createTest(
        ...
        switch (mode) {
            ...
            case NONE:
                ...
                break;
            case ASSERTED_ONLY:
                test = axiom -> !ontology.containsAxiom(axiom, Imports.INCLUDED,
