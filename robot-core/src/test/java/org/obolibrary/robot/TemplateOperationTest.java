...
public class TemplateOperationTest extends CoreTest {
    ...
    public void testTemplateStrings() throws Exception {
        ...
        ann = TemplateOperation.getStringAnnotation(checker,
                "A rdfs:label", "bar");
        assertEquals("Annotation(rdfs:label \"bar\"^^xsd:string)",
                ann.toString());

        ann = TemplateOperation.getTypedAnnotation(checker,
                "AT rdfs:label^^xsd:integer", "1");
        assertEquals("Annotation(rdfs:label \"1\"^^xsd:integer)",
                ann.toString());

        ann = TemplateOperation.getLanguageAnnotation(checker,
                "AL rdfs:label@en", "bar");
        assertEquals("Annotation(rdfs:label \"bar\"@en)", ann.toString());

        ann = TemplateOperation.getIRIAnnotation(checker,
                "AI rdfs:label", IRI.create("http://bar.com"));
    ...
    public void testChecker() throws Exception {
        ...
        ManchesterOWLSyntaxClassExpressionParser parser =
            new ManchesterOWLSyntaxClassExpressionParser(dataFactory,
                checker
